 ...
 */
class Cron_bu extends CI_Controller
{
    ...
    {
      // AKHIR BULAN
      $tanggal_awal_bulan = "2022-07-31";
      $tanggal_akhir_bulan = "2022-07-01";

      $tanggal_awal_3_bulan_lalu = "2022-04-01";
      $tanggal_akhir_3_bulan_lalu = "2022-06-30";
      ...
      (SELECT COALESCE(SUM(sub_a.open_limit), 0) FROM tb_fintech_facility_fee_sme sub_a WHERE sub_a.register_code = a.register_code AND "'.$tanggal_akhir_bulan.'" <= DATE(sub_a.end_date) AND "'.$tanggal_akhir_bulan.'" >= DATE(sub_a.start_date)) as sum_plafond,
      (SELECT sub_a.start_date FROM tb_fintech_facility_fee_sme sub_a WHERE sub_a.register_code = a.register_code AND "'.$tanggal_akhir_bulan.'" <= DATE(sub_a.end_date) AND "'.$tanggal_akhir_bulan.'" >= DATE(sub_a.start_date) ORDER BY sub_a.start_date ASC LIMIT 1) as sum_facility_periode_start,
      (SELECT sub_a.end_date FROM tb_fintech_facility_fee_sme sub_a WHERE sub_a.register_code = a.register_code AND "'.$tanggal_akhir_bulan.'" <= DATE(sub_a.end_date) AND "'.$tanggal_akhir_bulan.'" >= DATE(sub_a.start_date) ORDER BY sub_a.end_date DESC LIMIT 1) as sum_facility_periode_end,
      (SELECT sub_a.rate FROM tb_fintech_facility_fee_sme sub_a WHERE sub_a.register_code = a.register_code AND "'.$tanggal_akhir_bulan.'" <= DATE(sub_a.end_date) AND "'.$tanggal_akhir_bulan.'" >= DATE(sub_a.start_date) ORDER BY sub_a.rate DESC LIMIT 1) as sum_facility_fee,
      ...
      (
        SELECT COALESCE
          ( SUM( sub_a.loan_amount ), 0 )
        FROM
          tb_invoice_borrower_loan sub_a
        WHERE
          sub_a.register_code = a.register_code
        AND sub_a.loan_status IN ( "Disbursed", "Closed" )
        AND sub_a.loan_start_date <= "'.$tanggal_akhir_bulan.'"
      ) as sum_outstanding,
      ...
      CASE
      WHEN h.sum_dis_plan IS NULL THEN (SELECT COALESCE(SUM(sub_a.loan_amount)/3, 0) FROM tb_invoice_borrower_loan sub_a WHERE sub_a.register_code = a.register_code AND sub_a.loan_status IN ("Disbursed","default","Done") AND loan_start_date >= "'.$tanggal_awal_3_bulan_lalu.'"
      AND loan_start_date <= "'.$tanggal_akhir_3_bulan_lalu.'")
      WHEN h.sum_dis_plan = 0 THEN (SELECT COALESCE(SUM(sub_a.loan_amount)/3, 0) FROM tb_invoice_borrower_loan sub_a WHERE sub_a.register_code = a.register_code AND sub_a.loan_status IN ("Disbursed","default","Done") AND loan_start_date >= "'.$tanggal_awal_3_bulan_lalu.'"
      AND loan_start_date <= "'.$tanggal_akhir_3_bulan_lalu.'")
      ELSE h.sum_dis_plan
      END as sum_dis_plan,
      ...
      (
        ...
        WHERE
          sub_a.register_code = a.register_code
        AND sub_a.loan_status IN ( "Disbursed", "Closed", "Done" )
        AND MONTH(loan_start_date) = '.$bulan_saat_ini.'
        AND YEAR(loan_start_date) = '.$tahun_saat_ini.'
      ) as sum_dis_actual,

      h.sum_rep_payment_promise_date,
      (SELECT sub_b.payment_periode_date FROM tb_invoice_borrower_loan sub_a JOIN tb_invoice_borrower_payment_periode sub_b ON sub_b.id_borrower_loan = sub_a.id_borrower_loan WHERE sub_a.register_code = a.register_code AND sub_a.loan_status IN ( "Disbursed", "Closed" ) AND sub_b.payment_periode_status_principal IN ( \'Late, Not Yet Paid\', "default" ) AND sub_b.payment_periode_date <= "'.$tanggal_akhir_bulan.'" ORDER BY sub_b.payment_periode_date ASC LIMIT 1) as sum_rep_due_date,

      (
        SELECT
          sub_b.payment_periode_date
        FROM
          tb_invoice_borrower_loan sub_a
          JOIN tb_invoice_borrower_payment_periode sub_b ON sub_b.id_borrower_loan = sub_a.id_borrower_loan
        WHERE
          sub_a.register_code = a.register_code
        AND MONTH(sub_b.payment_periode_date) = '.$bulan_saat_ini.'
        AND YEAR(sub_b.payment_periode_date) = '.$tahun_saat_ini.'
        GROUP BY sub_a.register_code
        ORDER BY sub_b.payment_periode_date ASC LIMIT 1
      ) as sum_rep_due_date,

      CASE
      WHEN h.sum_rep_plan IS NULL THEN (SELECT COALESCE(SUM(sub_b.payment_periode_principal + sub_b.payment_periode_interest + sub_b.payment_periode_pinalty), 0) FROM tb_invoice_borrower_loan sub_a JOIN tb_invoice_borrower_payment_periode sub_b ON sub_a.id_borrower_loan = sub_b.id_borrower_loan WHERE sub_a.loan_status IN ("Disbursed","default","Done") AND sub_a.register_code = a.register_code AND sub_b.payment_periode_date >= "'.$tanggal_awal_bulan.'" AND sub_b.payment_periode_date <= "'.$tanggal_akhir_bulan.'")
      ...
      (
        ...
        FROM
          tb_invoice_borrower_loan sub_a
        JOIN tb_invoice_borrower_payment_periode sub_b ON sub_a.id_borrower_loan = sub_b.id_borrower_loan
        WHERE
          sub_a.register_code = a.register_code
        AND sub_a.loan_status IN ("Done" )
        AND MONTH(sub_b.date_payment) = '.$bulan_saat_ini.'
        AND YEAR(sub_b.date_payment) = '.$tahun_saat_ini.'
      ) as sum_rep_actual,
      ...
      ->join('tb_summary_sme h', 'a.register_code = h.register_code and h.sum_periode_month = '.$bulan_saat_ini.' and h.sum_periode_year = '.$tahun_saat_ini.'')
      ...
      foreach ($data as $key => $value) {
        $where = [
          'register_code' => $value->register_code,
          'sum_periode_month' => '.$bulan_saat_ini.',
          'sum_periode_year' => '.$tahun_saat_ini.'
        ];
        ...
        $check = $this->master_model->data('v1', '', 'tb_summary_sme', $where)->get()->row();
        if (!empty($check)) {
          $this->master_model->update($recap_summary, $where, 'tb_summary_sme', 'v1');
        }else{
          $this->master_model->save($recap_summary, 'tb_summary_sme', 'v1');
        }
      }
    ...
    public function calculate_target_disbursement()
    {
      ...
      $tanggal_awal_bulan = "2022-07-01";

      $bulan_saat_ini = "07";
      $tahun_saat_ini = "2022";

      $where = [
        'a.target_month' => $bulan_saat_ini,
        'a.target_year' => $tahun_saat_ini
      ];
      ...
      $bor_farmer = $this->master_model->data('v1', '
        ...
        (
          ...
          AND MONTH(sub_a.loan_start_date) = '.$bulan_saat_ini.'
          AND YEAR(sub_a.loan_start_date) = '.$tahun_saat_ini.'
      ...
      ', 'tb_fintech_register a')
        ...
        (
          ...
          WHERE
          	sub_a.register_code = a.register_code
          AND sub_a.loan_status IN ( "Disbursed","default","Done" )
          AND sub_a.loan_start_date < "'.$tanggal_awal_bulan.'"
        ) > 0
        ', NULL)
        ->where('a.register_status', 'Borrower')
        ->where('a.is_sme', 1)
      ...
      $hunter_actual = $this->master_model->data('', '
        ...
        ->where('c.bor_periode_month', $bulan_saat_ini)
        ->where('c.bor_periode_year', $tahun_saat_ini)
        ->get()->row()->hunter_actual;

      if (empty($target)) {
        $data = [
          'target_month' => $bulan_saat_ini,
          'target_year' => $tahun_saat_ini,
