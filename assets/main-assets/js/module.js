...
function _modal_update(module_id) {
    $.ajax({
        url: getUrl,
        type: 'POST',
        dataType: 'json',
        ...
        success: function (response) {
            if (response.success === true) {
                $("#formUpdate")[0].reset();
                $("#up_module_id").val(response.data.id);
                $("#up_module_name").val(response.data.name);
                $("#up_module_description").html(response.data.description);
...
$("#formInput").submit(function (e) {
    ...
    if (module_name.length >= 3 && module_name.length <= 30) {
        $.ajax({
            url: createUrl,
            type: 'POST',
            dataType: 'json',
...
$("#formUpdate").submit(function (e) {
    e.preventDefault();
    var module_name = $("#up_module_name").val();
